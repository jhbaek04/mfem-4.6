...
namespace mfem
{
	...
	class AutoDiffBasedBlockBoundaryIntegrator : public BlockNonlinearFormIntegrator
	{
		...
		std::function< mfem::Vector( mfem::Vector ) > mParamFunc;
		Vector mCoordinates;

		public:
		AutoDiffBasedBlockBoundaryIntegrator( const Array< int >& vsizes,
									  const std::function< mfem::Vector( mfem::Vector ) >& param_func ): 
										...
										const Array<Vector *> &elvect)
		{
			...
			for( int i = 0; i < ir.GetNPoints(); ++i )
			{
				...
				trans.Tranform( ip, mCoordinates );
				mParamVec = mParamFunc( mCoordinates );
	...
	class BlockNonlinearSystemSolver
	{
		...
		void solve( Vector& solution_coeff,
					const Array< Array<int>* >& bdr_attr_is_ess )
		{
			Array<Vector *> rhs( mSpaces.Size() );
			for( int s = 0; s < rhs.Size(); ++s ) rhs[s] = nullptr; // for now rhs is not set
			allocSolvers( bdr_attr_is_ess, rhs );
			Vector rhs_vec; // for now rhs is not set
			// rhs_vec.UseDevice( true );
			// solution_coeff.SyncMemory(solution_coeff);
		...
		void allocSolvers( const Array< Array<int>* >& bdr_attr_is_ess, Array<Vector *>& rhs )
		{
			deallocSolvers();
			...
			std::cout << "\033[33m" << "Set nonlinear form" << "\033[0m" << std::endl;
	...
	// 	Array<double> mData;
	// };
	// SparsityData ReadSparsityData(std::istream& in)
	// {
	// 	Array<int> sizes(2);
	...
	// 	Array<double> data;
	// 	sizes.Load( in, 1 );
	// 	printArray( sizes, "sizes");
	// 	row_offsets.Load( in, 0 );
	// 	printArray( row_offsets, "row_offsets");
	...
	//   RESIDUALSIZE: The size of the vector of residual. Must be the same as the state size for now.
	//   STATESIZE: The size of the state vector. The residual is written in terms of the state. See mfem::NavierStokesResidual as an example.
	//   PARAMSIZE: The number of physical parameters.
	template< template<typename,typename,typename,int,int,int > class RESIDUALCLASS, int DIM, int RESIDUALSIZE, int STATESIZE, int PARAMSIZE  >
	class AllDirichletProblemSolver
	{
		private:
		mfem::Mesh *mMesh;
		int mNumSpaces;
		...
		public:
		AllDirichletProblemSolver( mfem::Mesh *mesh, 
			...
			mParams( params.data(), params.size() ),
		...
		public:
		void setEssentialBC( const std::function< double( const int space_id, const int dof, const std::vector< double >& coordinates ) >& get_ebc )
		{
			mBdrAttrIsEssential.SetSize( mNumSpaces );
			std::cout << "\033[33m" << "mBdrAttrIsEssential.SetSize done" << "\033[0m" << std::endl;

			std::cout << "\033[33m" << "mMesh->" << "\033[0m" << std::endl;

			for( int s = 0; s < mNumSpaces; ++s ){
				mBdrAttrIsEssential[s] = new mfem::Array<int>( mMesh->bdr_attributes.Max() );
				*mBdrAttrIsEssential[s] = 1;
				printArray(*mBdrAttrIsEssential[s], "*mBdrAttrIsEssential[s]");

				mfem::Array<int> ess_tdof_list;
				// std::cout << "\033[33m" << "before mFESpaces[s]->GetEssentialTrueDofs" << "\033[0m" << std::endl;
				// std::cout << "\033[33m" << "mFESpaces[s]->Nonconforming() = " << mFESpaces[s]->Nonconforming() << "\033[0m" << std::endl;
				mFESpaces[s]->GetEssentialTrueDofs( *mBdrAttrIsEssential[s], ess_tdof_list );
				// std::cout << "\033[33m" << "mFESpaces[s]->GetEssentialTrueDofs done" << "\033[0m" << std::endl;
				
				const int num_funcs = mNodalCoordinates[s].Size() / DIM;
				for( int i = 0; i < ess_tdof_list.Size(); ++i ){
					const int dof = ess_tdof_list[i];
					const int func_id = dof % num_funcs;
					const std::vector< double > xyz ( { mNodalCoordinates[s][ 0*num_funcs + func_id ],
														mNodalCoordinates[s][ 1*num_funcs + func_id ],
														mNodalCoordinates[s][ 2*num_funcs + func_id ] } );
					mFuncCoeffs[ mOffsets[s] + dof ] = get_ebc( s, dof, xyz );
				}
			}
			std::cout << "\033[33m" << "setEssentialBC done" << "\033[0m" << std::endl;
				exit(EXIT_SUCCESS);
		}
		void solve()
		{
			mfem::BlockNonlinearSystemSolver< RESIDUALCLASS, DIM, RESIDUALSIZE, STATESIZE, PARAMSIZE > 
				nl_solver( *mMesh, mFESpaces, mParams.GetData() );

			std::cout << "\033[33m" << "set up BlockNonlinearSystemSolver" << "\033[0m" << std::endl;
				
			mfem::StopWatch timer;
			timer.Clear();
			timer.Start();
			nl_solver.solve( mFuncCoeffs, mBdrAttrIsEssential );
			timer.Stop();
...
int main(int argc, char *argv[])
{
	...
	// 	int vdim = 10;
	// 	mfem::Array<int> I(vdim+1); for( int i = 0; i < I.Size(); ++i ) I[i] = i;
	// 	mfem::Array<int> J(vdim); for( int i = 0; i < J.Size(); ++i ) J[i] = i;
	// 	mfem::Array<double> D(vdim); for( int i = 0; i < J.Size(); ++i ) D[i] = 1.0;
	// 	{
	// 		mfem::SparseMatrix p( I.GetData(), J.GetData(), D.GetData(), vdim, vdim, false, false, false );
	// 	}
	//   std::cout << "\nAfter brackets" <<  std::endl;
		// {
		// 	mfem::Vector diag_p( vdim );
		// 	diag_p = 1.0;
		// 	mfem::SparseMatrix p( diag_p );
		// 	mfem::DenseMatrix mat;
		// 	p.ToDenseMatrix( mat );
		// 	printMatrix( mat, "Loaded mat 2" );
		// }
		



	//   std::ifstream in("/mnt/c/Users/jhbae/Coreform/work/mfem/test/identity.txt");
	// //   mfem::SparseMatrix sp = mfem::LoadSparseMatrix( in );
	// //   std::cout << "Read sp." << std::endl;
	// //   mfem::DenseMatrix mat;
	// //   sp.ToDenseMatrix( mat );
	// //   std::cout << "Converted to dense mat." << std::endl;
	// //   printMatrix( mat, "Loaded mat" );

	  
	// 	std::cout << "LoadSparseMatrix begins." << std::endl;
	// 	// mfem::Array<int> sizes(2);
	// 	// mfem::Array<int> row_offsets;
	// 	// mfem::Array<int> col_indices;
	...
	// 	mfem::SparsityData sparsity_data = mfem::LoadSparsityData( in );

	// 	std::cout << "Read Arrays." << std::endl;

	// 	// mfem::SparseMatrix sp( row_offsets.GetData(), col_indices.GetData(), vals.GetData(), sizes[0], sizes[1] );
	// 	mfem::SparseMatrix sp( sparsity_data.mRowOffsets.GetData(), sparsity_data.mColIndices.GetData(), sparsity_data.mData.GetData(), sparsity_data.mSizes[0], sparsity_data.mSizes[1] );
	// 	mfem::DenseMatrix mat;
	// 	sp.ToDenseMatrix( mat );
	// 	printMatrix( mat, "Loaded mat" );
      ...
      // device.Print();
		...
		mfem::AllDirichletProblemSolver< mfem::ThermoNavierStokesResidual, space_dim, residual_size, state_size, param_size >
			solver( mesh, orders, var_dims, params, prolongation_file_names, device_config );
		// mfem::AllDirichletProblemSolver< mfem::ThermoNavierStokesResidual, space_dim, residual_size, state_size, param_size >
		// 	solver( &mesh, orders, var_dims, params, "cuda" );
		// mfem::AllDirichletProblemSolver< mfem::ThermoNavierStokesResidual, space_dim, residual_size, state_size, param_size >
		// 	solver( &mesh, orders, var_dims, params );
		...
		solver.setEssentialBC( [ & ]( const int space_id, const int dof, const std::vector< double >& coordinates ) -> double
								{	
									return exact_sols[ space_id ][ dof ];
								} );
