...
namespace mfem
{
	...
	class AllDirichletProblemSolver
	{
		...
		void setFunctionCoeffs()
		{
			...
			// mFuncCoeffs.SetSize( mTotalDofCounts );
			// mFuncCoeffs.UseDevice( false );
			// mFuncCoeffs.Update( mOffsets, MemoryType );
			mFuncCoeffs = 0.0;
		}
		void getNodalCoordinates()
		{
			...
			for( int space = 0; space < mNumSpaces; ++space ){
				...
				// std::cout << "mNodalCoordinates = " << std::endl;
				// for( int i = 0; i < mNodalCoordinates[ space ].Size(); ++i ){
				// 	std::cout << mNodalCoordinates[ space ][ i ] << ", ";
				// }
				// std::cout<< std::endl;
			}
...
// 		mfem::Mesh mesh( "/mnt/c/Users/jhbae/Coreform/work/mfem/test/test_mesh_sideset.e" );
...
// 			solver( &mesh, orders, var_dims, params, device_config );
// 		// mfem::AllDirichletProblemSolver< mfem::ThermoNavierStokesResidual, space_dim, residual_size, state_size, param_size >
...
// 		const double norm_rel = norm / norm_tot / mesh.GetNE();
